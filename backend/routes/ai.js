...
// Express raw body parser route for Whisper audio transcription
router.post('/transcribe', express.raw({ type: 'audio/webm', limit: '15mb' }), async (req, res) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Groq Whisper API returned status ${response.status}: ${errorText}`);
    }
