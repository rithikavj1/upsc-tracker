...
router.post('/create', auth, async (req, res) => {
  try {
    ...
    // ── Step 1: Get or create Razorpay customer ──────────────────────────────
    let customerId = user.razorpay_customer_id;

    if (!customerId) {
      try {
        const customer = await razorpay.customers.create({
          name: user.name || 'UPSC Aspirant',
          email: user.email,
        ...
        const desc = custErr?.error?.description || '';

        if (desc.includes('already exists')) {
          // Customer exists in Razorpay but not saved in our DB
          // Fetch all customers and match by email
          try {
            ...
            // If still not found, just proceed without customerId — it's optional
          } catch (fetchErr) {
            console.error('Customer fetch error:', fetchErr);
            // Continue without customerId — subscriptions work without it
          }
        } else {
          console.error('Customer create error:', custErr);
          // Non-duplicate error — still proceed, customerId optional
        }
      }
    }

    // ── Step 2: Resolve plan ID ───────────────────────────────────────────────
    const planType = req.body.plan === 'yearly' ? 'yearly' : 'monthly';
    ...
    if (!plan) {
      return res.status(400).json({
        error: `Plan ID not configured for: ${planType}. Check RAZORPAY_PLAN_ID_${planType.toUpperCase()} in environment variables.`
      });
    }
    ...
    const subscriptionPayload = {
      ...
      customer_notify: 1,
      quantity: 1,
      total_count: 12, // 1 year max (safe well under Razorpay's 100 limit)
      addons: [{
        item: {
    ...
    // Only attach customer_id if we have one
    if (customerId) {
    ...
    await pool.query(
      `UPDATE users SET razorpay_subscription_id=$1, subscription_status='pending' WHERE id=$2`,
    ...
    console.error('Subscription create error:', JSON.stringify(err));
...
router.post('/pause', auth, async (req, res) => {
  try {
    ...
    await razorpay.subscriptions.pause(user.razorpay_subscription_id, { pause_at: 'now' });
...
router.post('/resume', auth, async (req, res) => {
  try {
    ...
    await razorpay.subscriptions.resume(user.razorpay_subscription_id, { resume_at: 'now' });
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    if (event === 'subscription.cancelled') {
      await pool.query(
        `UPDATE users SET subscription_status='cancelled' WHERE razorpay_subscription_id=$1`,
        [subId]
      );
    }
